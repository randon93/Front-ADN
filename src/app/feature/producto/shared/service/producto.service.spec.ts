...
describe('ProductoService', () => {
  ...
  const patch = environment.urlBase + '/producto';
  const apiEndpointProductoConsulta = `${patch}`;
  const apiEndpointProductos = `${patch}/delete`;
  ...
  it('deberia listar productos', () => {
    const dummyProductos = [
      new Producto('1', 'BATMAN', 1000),
      new Producto('15', 'TOMYJERRY', 55),
      new Producto('21', 'tolosa-man', 1233)
    ];
  ...
  it('deberia crear un producto', () => {
    const dummyProducto = new Producto('1', 'Producto 1', 100);
